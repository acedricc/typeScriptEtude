...
function codeurGroupTherapy (civilite: string, nom: string, age: number, job?: string): string {
    return `<br>Bonjour, je suis ${civilite} ${nom}, j'ai ${age} ans, ${job ? `je suis ${job}` : ''} et je suis accro au code!!` +
    `<br><br> - Bonjour ${civilite} ${nom}`;
}

let civilite: string = 'Monsieur';
let nom: string = "Capone";
let age: number = 47;
let job: string = "cambrioleur";

document.write(codeurGroupTherapy(civilite, nom, age));

let uneVariable: any;
uneVariable = "unechine";
uneVariable = 34;
uneVariable = true;
